...
def get_by_id(deputado_id: int, ano: int = Query(None, description="Ano do database"), 
              session: Session = Depends(get_session)):
    
    statement = select(Deputado).where(Deputado.id == deputado_id)
...
def get_all(
    ...
    items_response = [
        DeputadoResponse.from_model(dep)
